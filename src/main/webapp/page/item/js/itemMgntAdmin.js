            ...
            items: [{
				...
				listeners : {
					click : {
						element : 'el',
						fn : function() {
							window.location.href = rootPath + "/itemAdmin/addItemPageAdmin?type=add";
						}
					}
				}
            }]
...
function buttonRender(value, meta, record, rowIndex, colIndex, store) {
    ...
    returnValue += '<em class="modify-ico" title="修改" onclick="modifyItemAdmin('+ itemId +')"></em>'+
                   '<em class="del-ico" title="删除" onclick="deleteItemAdmin('+ itemId +')"></em>';
...
function deleteItemAdmin(itemId) {
	  if (confirm('是否要删除该用户')) {
	    $.ajax({
	       url : rootPath + "/itemAdmin/deleteItemAdmin",
	       type : 'GET',
	       dataType : 'json',
	       data : {
	    	   itemId : itemId
...
//定义修改事件
function modifyItemAdmin(itemId){
	window.location.href = rootPath + "/itemAdmin/addItemAdminPage?type=modify&itemId=" + itemId;
}
